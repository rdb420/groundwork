    ...
    <div className="pipeline">
      ...
      <dl className="tiles">
        {Object.entries(s.services).map(([k, v]) => (
          <div key={k}><dt>{SERVICE[k] ?? k}</dt><dd className="small">{v === true ? "Set up" : v === false ? "Not set up" : String(v)}</dd></div>
        ))}
      </dl>
      <h2>Files</h2>
      <ul className="rows">{Object.entries(s.files).map(([k, v]) => <li key={k}><span>{k}</span><span className="quiet">{v} file{v === 1 ? "" : "s"}</span><span /></li>)}</ul>
        ...
        <ul className="rows">
          {s.attention.map((a) => (
            <li key={a.document_id}>
              ...
              <span className={`status ${a.status === "failed" ? "s-failed" : ""}`}>{a.status === "partial" ? "Partly done" : a.status === "skipped" ? "Skipped" : "Failed"}</span>
